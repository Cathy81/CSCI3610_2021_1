...
<title>Medication</title>
  ...
  $MedStartDate=date("Y-m-d", strtotime($_POST["MedStartDate"]));
  $MedEndDate=date("Y-m-d", strtotime($_POST["MedEndDate"]));
  $MedNote=$_POST["MedNote"];
  $query="UPDATE tblmeds SET fkMedType=\"$fkMedType\",MedName=\"$MedName\",Dose=\"$Dose\",Frequency=\"$Frequency\",TakenWhen=\"$TakenWhen\",MedStartDate=\"$MedStartDate\",MedEndDate=\"$MedEndDate\",MedNote=\"$MedNote \" WHERE MedID= \"$MedID\"";
  $result=$conn->query($query);
    if(!$result) 
      {
  //      echo "query is not correct!";
        die ("query can not be executed!".$conn->error);
      }
    echo "<h4>The record has been changed!</h4>";
